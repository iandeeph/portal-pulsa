...
#ISAT
#===============================================================================
ISATResult=($(mysql db_agen_pulsa -h$HOST -u$USER -p$PASSWORD -Bse "select namaProvider, noProvider, host, span, hargaPaket, caraCekPulsa from provider where namaProvider not like '%XL%' order by length(namaProvider), namaProvider;"))
cntISATElm=6
cntISAT=${#ISATResult[@]}
			...
			while [[ $attempt -le $maxAttempt && "$cekBerhasil" != "berhasil"  ]]; do
				...
				if [[ "$cekString2" = "Pulsa" ]] || [[ "$cekString3" = "Pulsa" ]]; then
					...
					if [[ $attempt == $maxAttempt ]]; then
						slackText="Indosat ${ISATNo[$isatnum]}.. Lokasi : ${ISATHost[$isatnum]} - ${ISATSpan[$isatnum]} Cek Gagal...\n$USSDReplyISAT"
						curl -X POST -H 'Content-type: application/json' --data '{"text": "```'"$slackText"'```", "channel": "'"$CHANNEL"'", "username": "'"$USERNAME"'", "icon_emoji": "'"$ICONEMOJI"'"}' https://hooks.slack.com/services/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
